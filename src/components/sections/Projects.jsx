        ...
        <div className="container mx-auto max-w-6xl">
            ...
            {projectsData.map((project, index) => (
                <motion.div
                    ...
                    viewport={{ once: true }}
                    className={`bg-card border border-border rounded-lg overflow-hidden shadow-lg hover:shadow-xl transition-shadow container-${theme}`}
                >
                <div className="p-6">
                    ...
                    {project.tech.map((tech, i) => (
                        <span
                        key={i}
                        className={`px-2 py-1 bg-secondary text-secondary-foreground rounded text-xs box-skills-${theme}`}
                        >
                        {tech}
                        </span>
                    ))}
                    ...
                    <button className={`flex items-center border rounded-[8px] p-2 px-3 text-sm font-medium container-${theme}`} href={project.github} target="_blank" rel="noopener noreferrer">
                        <FiGithub className="h-4 w-4 mr-1" />
                        {t.projects.viewCode}
                    </button>
                    <button className={`flex items-center rounded-[8px] p-2 px-3 text-sm font-medium button-view-${theme}`} href={project.demo} target="_blank" rel="noopener noreferrer">
